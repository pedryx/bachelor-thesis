...
\begin{enumerate}
    ...
    \item \textbf{Hra by neměla využívat častého přidávání a odebírání komponent:} ECS knihovny se dají rozdělit podle způsobu ukládání komponent. Rozlišují se převážně knihovny založené na \textit{arch type} a knihovny založené na \textit{sparse set}. Knihovny založené na \textit{arch type} většinou realizují přidávání a odebírání komponent v čase O(n). Knihovny založené na \textit{sparse set} jsou na tom lépe a většinou to realizují v O(1), ovšem jejich nevýhodou bývá lehce pomalejší iterace přes komponenty v porovnáním s knihovnami založenými na \textit{arch type}. Více si o tomto řekneme později konkrétně v sekci \ref{section:component-storage}.
