...
Některe knhovny ktere budeme merit vyzaduji aby komponenty byli unmanaged typy. Na strance xxx je mozne videt ze definice unmanaged typu je
...
My potrebujeme nektere unmanaged typy na komponentech, zejmena IENtity a pole. Kde a proc.

Pro vyreseni mame managed data managery viz obrazek. Abstraktni metody createdatainstance a createempty. Je mozne vlozit instanci do managera, to nam vrati id. Je mozne pomoci id ziskat data z managera. Je nutne nezapmenout uvolnit data z managera aby nebyl memmory leak. V nasi hre to resime pres deathsystem.

obrazek

Jednotlive instance manageru uchovany v game. game k nim dava pristup. Jednotlive instance mozno videt na obrazku.
...
Měření je druhým C\# projektem našeho C\# řešení. S použitím naší hry měří výkon jednotlivých ECS knihoven. V sekci~\ref{benchmark-framework} jsme se rozhodli, že pro toto měření použijeme \textit{BenchmarkDotNet}. Celé měření se dělí na dvě fáze a to přípravy a samotného měření.

Pro měření máme definované následující konstanty: \texttt{seed}, \texttt{deltaTime}, \texttt{setupIterationCount} a \texttt{benchmarkIterationCount}. \texttt{seed} je seed, který používáme pro náhodné generování čísel. Stejným seedem pro každé měření zajistíme konzistentnost. \texttt{deltaTime} obsahuje čas v sekundách. Tento čas používáme pro odsimulovaní jednoho kroku naší hry (jedné iterace \textit{game loop}, který byl přiblížen v sekci~\ref{game-loop}). \texttt{setupIterationCount} nám udává počet kroků odsimulování naší hry během fáze přípravy a poté \texttt{benchmarkIterationCount} udává počet kroků odsimulování hry během fáze měření.

V C\# projektu máme definovaný test reprezentovaný třídou \texttt{ECSBenchmarks}. Tento test obsahuje jediný vstupní parametr a tím je \texttt{ECSFactoryType} typu \texttt{Type}. Tímto parametrem jsou předávány typy dědicí od \texttt{ECSFactory}, které reprezentují jednotlivé ECS knihovny.
...
Během fáze měření používáme tu samou instanci hry vytvořenné během fáze přípravy. Na této instanci v cyklu opět zavoláme metodu \texttt{UpdateOnce}. Počet opakování tohoto cyklu je dán konstantou \texttt{benchmarkIterationCount}.
