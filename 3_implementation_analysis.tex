...
Pro tvorbu naší hry bude nutné zvolit herní engine nebo framework. Jak jsme si říkali v sekci~\ref{sec:abstract-layer-intro} naše hra bude namísto konkrétní ECS knihovny, pracovat pouze s abstrakční vrstvou. Budeme tedy chtít aby pro pro naši hru, běžící se zvoleným enginem či frameworkem, bylo možné specifikovat konkrétní implementaci abstrakční vrstvy, se kterou bude hra pracovat. Dále tuto hru budeme chtít použít pro vytvoření porovnání různých ECS knihoven. Budeme tedy požadovat aby ji bylo možné spouštět z jednoho měření s různými implementacemi abstrakční vrstvy, které budou odpovídat daným ECS knihovnám.
...
\xxx{rozbor jednotlivých ECS knihoven?}
...
Náš terén budeme reprezentovat texturou. Každý pixel této textury bude mít barvu odpovídající danému biomu. Například biom vody může mít modrou barvu a biom louky zelenou barvu. Poté všechny modré pixely v naší textuře budou odpovídat biomu vody a všechny zelené biomu louky.
...
\begin{enumerate}
    ...
    \item \textbf{Spojitost:} Budeme chtít aby terén byl v jistém smyslu spojitý. Konkrétně chceme aby jednotlivé biomy tvořili nepřerušované oblasti.
    ...
    \item \textbf{Žádné větší vizuální artefakty:} Nutně nevyžadujeme hodne dobře vypadající terén, ale zároveň nechceme větší vizuální artefakty.
\end{enumerate}

\subsection{Generování terénu}
Pro generování terénu se běžně používají výškové mapy vygenerované za pomoci šumové funkce. Proto si nejprve jednotlivé šumové funkce představíme a následně jednu z nich zvolíme. Poté si představíme způsob, kterým zvolenou šumovou funkci budeme používat pro generování terénu. Mezi nejpoužívanější šumové funkce patří:

\begin{enumerate}
    \item \textbf{Perlinův šum:} Jedná se o známou šumovou funkci. Tato šumová funkce je rychlá a také jednoduchá na implementaci. Nejčastěji se používá ve druhé a třetí dimenzi. Více o Perlinovu šumu je možné se dočíst v diplomové práci \textit{Terrain synthesis using noise}~\cite{TerrainNoise} od \textit{Tuoma Hyttinena}.

    \item \textbf{Simplexový šum:} \textit{Simplexový šum} je velmi podobný \textit{Perlinovu šumu}. Oproti \textit{Perlinovu šumu} je rychlejší a produkuje výsledky, které vypadají více přirozeně. Více o \textit{Simplexovém šumu} je možné se dočíst v již zmíněné diplomové práci \textit{Terrain synthesis using noise}~\cite{TerrainNoise}.
\end{enumerate}

My si pro tuto práci zvolíme \textit{Simplexový šum}, jelikož je rychlejší a produkuje více přirozené výsledky. Konkrétně jej budeme používat ve druhé dimenzi.

Nyní za pomoci \textit{Simplexového šumu} vygenerujeme výškovou mapu. Pro generování výškové mapy se často používá Fraktální součet nebo Turbulence. Jedná se o techniky pro generování více komplexních vzorů. O obou technikách je možné se více dočíst v již zmíněné diplomové práci \textit{Terrain synthesis using noise}~\cite{TerrainNoise}. Obě techniky jsou pro nás dobré, ale autorovi práce se více líbí vzhled terénu vygenerovaný Fraktálním součtem, proto jej zvolíme.

Nyní zbývá pomocí výškové mapy vygenerovat daný terén. Pro generování zvolíme přímočarou a běžně populární techniku. Jak jsme si říkali náš terén bude textura, kde barva jednotlivých pixelů bude odpovídat danému biomu. Každý biom bude mít definovaný výškový interval a barvu. Pro souřadnice každého pixelu si necháme vygenerovat výšku z výškové mapy. Poté na základě této výšky a výškových intervalů jednotlivých biomů získáme příslušný biom. Poté barvu daného pixelu nastavíme na barvu daného biomu. O této technice a také o tom jak ji použít pro 3D terén je možné se více dočíst ve článku \textit{Making maps with noise functions}~\cite{TerrainHeightMap} ze stránky \textit{Red Blob Games}.

% \begin{figure}
%     \centering
%     \includegraphics[scale=0.15]{img/perlin}
%     \caption{TODO}
% \end{figure}

\subsection{Generování terénu na CPU vs. GPU}
Za použití techniky z minulé sekce jsme získali vygenerovaný terén. Tato technika splňuje požadavky na rychlost a spojitost, ovšem zbylé dva požadavky (možnost navigace a absence vizuálních artefaktů) mohou být porušeny v závislosti na tom zda terén budeme generovat na CPU nebo na GPU. Proto si nyní oba přístupy rozebereme.
