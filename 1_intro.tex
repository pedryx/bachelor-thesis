...
Jedním ze způsobů jak si vývojáři můžou vybrat ECS knihovnu je použít porovnání. CSharpECSComparison~\cite{CSharpECSComparison} je projekt na platformě GitHub, který porovnává populární ECS knihovny pro C\# na základě funkcionalit, vlastností a závislostí. Vývojář by si tedy mohl říci, které vlastnosti a funkcionality by od takové knihovny chtěl a na základě toho si vybrat. Ovšem tento projekt nám nedává žadné informace o výkonu jednotlivých knihoven.
...
Jedním ze způsobů, jak by bylo možné lépe porovnat výkon jednotlivých ECS knihoven, by bylo vytvoření hry, na které by bylo možné jednotlivé knihovny porovnávat. Taková hra by musela být nezávislá na konkrétní ECS knihovně. Také, aby se na ní lépe provádělo měření, by měla obsahovat netrivialně velký svět s větším počtem entit. Jeden z cílů této práce bude vytvoření takovéto hry.
