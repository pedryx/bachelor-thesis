...
\begin{figure}
    \centering
    \includegraphics[width=0.66\linewidth]{img/transition.pdf}
...
Je lehké nahlédnou, že obě řešení přidávají netriviální složitost navíc do definice chování pro naše vesničany.
...
Je lehké nahlédnou, že rozšiřitelnost a modularita \textit{stromů chování} jsou opravu silné vlastnosti. Proto také \textit{stromy chování} použijeme pro definici chování pro naše vesničany.
...
% TODO:
% navigace
%     - prvne jak to budeme delat a co od toho pozadujeme
%     - pote analyzovat
...
Vesničané v naší hře budou často potřebovat nalézt cestu v námi vygenerovaném terénu k nějaké entitě. Budou například chtít dojít ke skladišti nebo ke svému pracovnímu místu. Jelikož k hledaní cesty bude docházet poměrně často budeme chtít aby bylo dostatečně rychlé. Dále budeme chtít aby cesty působili přirozeně. Příklad nepřirozené cesty je ceste ve které vesničan dělá zbytečně velkou zacházku nebo cesta ve které vesničan nejde přímo ke svému cíli i přesto, že mezi ním a cílem není žádná překážka. Příklady vhodné cesty a několika nevhodných cest je možné vidět na obrázku~\ref{fig:path}.

\begin{figure}
    \centering
    \includegraphics[width=0.66\linewidth]{img/path.png}
    \caption{Příklad cest z bodu A do bodu B. Fialové cesty jsou nežádané nepřirozeně vypadající cesty. Červená cesta je žádaná přirozeně vypadající cesta.}
    \label{fig:path}
\end{figure}

% jak ji budeme provadet
%   - A* (protoze se bezne pouziva, produkuje nejkratsi cesty a je rychli)
%   - vysledek nevypada prirozene kvuli mrizce, proto provadime zkrasleni cesty
%   - nespojita navigace ve spojitem terenu
% jak ziskame mrizku
Náš terén je dvourozměrný prostor, konkrétně si jej budeme reprezentovat jako mřížku. Tuto mřížku si budeme reprezentovat jako pole, kde každý prvek tohoto pole ponese informaci o tom v jakém biomu se nachází. Pro výpočet vzdálenosti mezi dvěma body v naší mřížce zvolíme Manhattanskou metriku. Pro hledání nejkratších cest použijeme algoritmus A*, jelikož se jedná o známý a rychlí algoritmus pro hledání nejkratších cest. 

Náš terén je spojitý, ovšem prostor používaný pro navigace v něm máme reprezentovaný pouze jako mřížku. Výsledné cesty vyprodukované A* algoritmem by tedy v původním terénu nevypadali přirozeně, proto po skončení A* algoritmu provádíme ještě jeden krok, při kterém tyto cesty zjednodušujeme.
