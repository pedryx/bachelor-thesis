...
Parametr \texttt{deltaTime} je nastaven na 1/60s a to odpovídá 60 FPS (snímkům za sekundu). Poté parametr \texttt{setupIterationCount} odpovídá zhruba 30 sekundám simulace a parametr \texttt{benchmarkIterationCount} je přibližně 1 minuta simulace.

\subsection{První měření}
První měření bylo spuštěno s velikostí světa 8192x8192. Tento svět obsahoval \xxx{X} entit. Výsledky tohoto měření:
...
Druhé měření bylo spouštěno s velikostí světa 16384x16384. Tento svět obsahoval \xxx{X} entit. Výsledky tohoto měření:
...
Třetí měření bylo spouštěno s velikostí světa 32768x32768. Tento svět obsahoval \xxx{X} entit. Výsledky tohoto měření:
