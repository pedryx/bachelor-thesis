...
Jedním z cílů této práce je udělat hru. V sekci~\ref{sec:ecs-libs} jsme si říkali, že danou hru budeme chtíp použít pro měření ECS knihoven, které používají programovací jazyk C\#. Proto C\# také zvolíme jako programovací jazyk pro naši hru.

Pro tvorbu naší hry bude nutné zvolit herní engine nebo framework.
% --------
% volba framework/engine
%
% - potrebujeme udelat hru pro csharp
% - mohli bychom zvolit herni engine jako unity nebo godot
% - game loop odbocka
% - enginy nas od game loop odstinuji
% - framework je pro nas vhodnejsi protoze lepsi low level control
% - nabizi se MonoGame, FNA, dalsi?
% - background (xna)
% - syntaxe vicemene stejna
% - monogame: lepsi dokumentace, dalsi vyhody?
...
\begin{enumerate}
    \item \textbf{\texttt{EntityProcessor}:} Tento typ bude mít na sobě již zmiňovanou funkci, nazvěme ji \verb|Process|, pro zpracování jedné entity.
...
Jednotlivé ECS knihovny mají různé požadavky na typech jednotlivých komponent. Některé chtějí aby tyto typy byli třídy, jiné zase chtějí aby tyto typy byli struktury a jiné povolují kombinaci obou variant.
...
Jak již víme naše hra bude obsahovat vesničany. Každý vesničan bude reprezentován entitou a bude vykonávat své akce na základě jednoduché umělé inteligence. Ovšem způsobů, kterými lze umělou inteligenci aneb chování dané entity definovat je více, proto si je nyní rozebereme. Mezi tyto způsoby patří:
